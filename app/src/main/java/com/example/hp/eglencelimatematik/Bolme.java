...
public class Bolme extends AppCompatActivity implements MediaPlayer.OnCompletionListener, TextToSpeech.OnInitListener{
                    ...
                    synchronized(this){
                        ...
                        try {
                            ...
                            Toast toast = Toast.makeText(getApplicationContext(),
                                    "Cihazınız bu uygulamayı desteklemiyor..", Toast.LENGTH_LONG);
                            toast.show();
                        }
                    }
                }
...
// Don't forget to shutdown!
    ...
    {
        if (BolmeSeviye.kolayBasildiMi==true){
            sayi1 = r.nextInt(31);
            sayi2 = r.nextInt(3)+1;
            aralik=30;
        ...
        else if (BolmeSeviye.ortaBasildiMi==true){
            sayi1 = r.nextInt(31)+20;
            sayi2 = r.nextInt(22)+4;
            aralik=20;
            while ((sayi1 < sayi2) || (sayi1 % sayi2 != 0))
            {
                sayi1 = r.nextInt(31)+20;
            ...
            sayi1 = r.nextInt(51)+50;
            sayi2 = r.nextInt(46)+5;
            aralik=25;
            while ((sayi1 < sayi2) || (sayi1 % sayi2 != 0))
            {
                sayi1 = r.nextInt(51)+50;
        ...
        dogruSecenek = r.nextInt(3);// şıklara dogru cevabı rastgele atamak için

        String islemMetni = String.valueOf(sayi1) + " / " + String.valueOf(sayi2) + " = ?";
        txtIslem.setText(islemMetni);

        soruText = sayi1text +" bölü "+ sayi2text + " işleminin cevabı aşağıdakilerden hangisidir ?";
        //speakOut(soruText);

        dogruDeger = islemiHesapla(sayi1, sayi2);
        int yanlisDeger1 = r.nextInt(aralik);
        int yanlisDeger2 = r.nextInt(aralik);
        ...
        switch (dogruSecenek)// 1 den 3 e kdr random alınan deger
        {
            case 0:
        ...
        switch (requestCode)
        {
            case RESULT_SPEECH:
                if (resultCode == RESULT_OK && data != null)
                {
                    ...
                    if (text.get(0).equals(String.valueOf(dogruDeger)) || (text.get
                            (0).equals("bir") && dogruDeger == 1))
                    {
                        mediaPlayer = MediaPlayer.create(getApplicationContext
                                (),R.raw.applause3);// cevap dogru ise alkıs
                        mediaPlayer.start();
                    }
                    else
                    {
                        mediaPlayer = MediaPlayer.create(getApplicationContext
                                (),R.raw.wrong);// cevap yanlış ise yanlış sesi
                        mediaPlayer.start();
                    }
                }
        }
        yourthread=  new Thread(){
            ...
            public void run(){
                try {
                    synchronized(this){
                        wait(2500);
                        Intent intent = new Intent(getApplicationContext(), Bolme.class);
